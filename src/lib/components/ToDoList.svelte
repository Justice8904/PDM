...
{#each tarefas as tarefa, i}
  <div class="input-group mb-1 {tarefa.status == 1 ? 'text-decoration-line-through' : ''}">
    ...
    {:else}
      ...
      <button class="btn btn-danger input-group-text" aria-label="excluir" data-bs-toggle="modal" data-bs-target="#alertaModal" onclick={() => excluirTarefa(i)}><i class="bi bi-trash"></i></button>
    {/if}
  </div>
{/each}